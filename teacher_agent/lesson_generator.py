...
def clean_cn_punctuation(text: str) -> str:
    ...
    cleaned = re.sub(
        r"围绕\s+([^“”\n]+?)\s+展开[。.]?\s*[”\"]",
        lambda match: f"围绕“{match.group(1).strip()}”展开。",
        cleaned,
    )
    ...
    cleaned = re.sub(r"基于\s*STM32", "基于 STM32", cleaned, flags=re.IGNORECASE)
    return cleaned
