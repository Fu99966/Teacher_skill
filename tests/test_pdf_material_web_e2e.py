...
def test_uploaded_pdf_material_reaches_web_agent_generation(monkeypatch, tmp_path):
    ...
    assert payload["material_extraction"] == extraction
    assert payload["material_used"] is True
...
def test_scanned_pdf_material_reports_ocr_warning_to_web_client(monkeypatch, tmp_path):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "")
    _patch_web_paths(monkeypatch, tmp_path)
    ...
    assert status == 200, payload
    extraction = payload["material_extraction"]
    assert extraction["text"] == ""
    assert extraction["char_count"] == 0
    assert any("OCR" in warning for warning in extraction["warnings"])
    assert payload["material_used"] is False
    assert payload["is_generic_material"] is True
