...
def _post_multipart(base_url: str, path: str, fields: dict[str, str]) -> tuple[int, dict[str, Any]]:
    boundary = "----teacher-skill-v16-export-regression"
    chunks: list[bytes] = []
...
def test_v16_final_docx_blocks_prompt_material_and_generic_prep(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert sum(1 for keyword in prep_hits if keyword in compact) >= 6
        assert "多媒体课件、学习任务单、板书材料" not in compact
    finally:
        server.shutdown()
        server.server_close()
