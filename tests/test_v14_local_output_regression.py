...
AGENT_REQUEST = "帮我生成一份 24物联网1班《PCB板设计》的实训课教案，适合项目式教学，课时32课时。"
...
def _post_multipart(base_url: str, path: str, fields: dict[str, str]) -> tuple[int, dict[str, Any]]:
    boundary = "----teacher-skill-v14-regression"
    chunks: list[bytes] = []
...
def test_v14_local_system_export_keeps_pcb_title_and_clean_material(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert preview["agent_task"]["title"] == "PCB板设计"
        ...
        assert data["generation_backend"] != "deepseek_strict_failed"
        fields = data["fields"]
        assert fields.get("lesson_title") == "PCB板设计"
        assert fields.get("teaching_process")
        assert fields.get("teaching_method")
        # Simulate a stale/polluted browser preview where the title field was overwritten
        # by the teaching_method label. Export must repair it before naming/filling Word.
        fields = dict(fields)
        fields["lesson_title"] = "教学方法的运用"
        ...
        assert "PCB板设计" in export["output_name"]
        assert "教学方法的运用" not in export["output_name"]
        ...
        assert "课题 教学方法的运用" not in compact
        assert "《教学方法的运用》" not in compact
        assert "帮我生成一份 24物联网1班《PCB板设计》" not in compact
        assert "PCB设计项目任务书" in compact or "物联网应用技术专业课程要求" in compact
        assert "教学方法的运用" in compact
        assert "项目教学法" in compact or "任务驱动" in compact
        assert "32课时" in compact
